    ...
    <section :class="[
      ...
      <div class="flex items-center justify-between">
        ...
        <nav class="hidden lg:flex space-x-6 items-center text-slate-200" aria-label="Desktop Navigation">
          ...
          <router-link to="/team-dev" role="menuitem">Team Dev</router-link>
        </nav>
        ...
        <div class="hidden lg:flex space-x-6 items-center">
          <div class="relative">
            ...
            <ul v-if="suggestions.length"
              class="absolute shadow-lg rounded-mg w-full p-2 z-10 bg-color-thirty/70" role="listbox" aria-labelledby="search">
              <li v-for="(name, index) in suggestions" :key="index" @click="navigateToGame(name)"
                class="hover:bg-gray-500 px-4 py-2 text-gray-200 cursor-pointer" role="option" :aria-selected="index === 0">
                {{ name }}
              </li>
            </ul>
          </div>
          <a href="https://github.com/FEPT07/GSL_Team_App" target="_blank" rel="noopener noreferrer"
            class="text-white-700 hover:opacity-90 flex items-center space-x-2" aria-label="Go to GitHub repository">
      ...
      <!-- Mobile Menu -->
      <nav v-if="isMobileMenuOpen" class="lg:hidden bg-color-secondary py-4 space-y-4 text-slate-200" aria-label="Mobile Navigation">
